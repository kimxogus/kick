---
name: kick
description: kick 등록과 제품 상세 페이지 생성을 돕는다. 사용자가 "kick 스킬을 참고해서 내 제품 올려줘"처럼 요청하면 저장소 문서를 읽어 제품 핵심을 추출·정제·분류하고, 공식 kick 서비스에 등록한 뒤 상세 페이지 URL을 알려준다.
---
...
8. 공식 kick 서비스의 `POST https://kick-web-ebon.vercel.app/api/products`에 페이로드를 보낸다.
9. 응답의 `detailUrl`이 `/products/<slug>`이면 `https://kick-web-ebon.vercel.app/products/<slug>` 형태의 전체 URL로 사용자에게 안내한다. 사용자가 그 URL로 들어가 확인하는 것으로 끝낸다.
...
- `maker.profileUrl`은 메이커 프로필 URL일 때만 사용한다. 제품 홈페이지 URL 대체값으로 쓰지 않는다.
...
- 등록은 공식 kick 서비스에 쓰기 요청을 보낸다.
- 테스트 제품을 실제 공식 URL에 등록하면 배포 DB에 데이터가 남을 수 있으므로, 사용자가 명시적으로 요청한 제품만 등록한다.
