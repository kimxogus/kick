...
Next.js App Router 기준 화면은 아래 5개를 우선한다.

| 페이지 | route | 한 줄 역할 |
| --- | --- | --- |
| Weekly board | `/` | 이번 주 제품 탐색, 검색, 태그 필터, vote, newsletter 진입점 |
| Product 소개 | `/products/[slug]` | Skill/MCP가 만든 런칭 콘텐츠와 제품 상세 정보 |
| Maker assist | `/maker` | 제품 분석, 홍보 산출물 생성, 등록 후보 준비 |
| Submission preview | `/submissions/[id]` | 제작자가 저장한 제출 후보 preview |
| Contest | `/contest` | 공개 contest 목록을 읽기 전용으로 탐색 |

Contest 생성/운영, 댓글, 창작자 메시지는 데모 확장 아이디어로 유지하되 MVP 필수 구현 계약에는 포함하지 않는다.
...
- **완성도**: 실제 서비스 기반 초기 콘텐츠로 빈 화면을 피한다.
...
- 등록 후보 미리보기와 제출 후보 저장 버튼.

어필:

- **참신성**: 글 작성 보조를 넘어 등록 후보 준비까지 이어진다.
...
탐색자가 특정 주제의 제품 묶음을 훑어볼 수 있는 읽기 전용 화면이다.

내용:

- 공개 contest 제목, 주최, 설명, 기간, 상태.
- 참여 제품 수와 대표 제품 링크.
- Weekly board와 Maker assist로 돌아가는 사용자용 이동 링크.
...
- Contest 운영: 개최/상금/마감 기반 등록이 가능하다는 비전을 보여준다. MVP에서는 결제와 상세 운영 흐름을 구현하지 않는다.
...
- 실제 서비스 기반 초기 콘텐츠를 사용해 콘텐츠 밀도를 확보한다.
