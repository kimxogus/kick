...
describe("kick MVP backend service", () => {
  ...
  it("Skill 제품 등록 payload를 published 제품과 launch로 변환한다", async () => {
    ...
    expect(created.product.status).toBe("published");
    expect(created.product.websiteUrl).toBe("#");
    expect(created.product.tags).toEqual(["AI", "Productivity"]);
    expect(created.product.makers[0]?.profileUrl).toBe("https://example.com/demo-team");
    expect(created.launch.voteCount).toBe(0);
